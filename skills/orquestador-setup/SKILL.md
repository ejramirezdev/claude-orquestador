...
3. **Bloque en `CLAUDE.md`** del proyecto (créalo si no existe; si el bloque ya existe, reemplázalo):
   ...
   (`scripts/revisar-recursos` de la skill `orquestar`); pregunta solo lo que no se pueda deducir.
...
5. **Git.** El proyecto es un repositorio con al menos un commit en la rama principal y `git worktree
   list` funciona. Si hay worktrees viejos, corre `scripts/limpiar-worktrees.sh` (sin `--aplicar`) y
   muéstrale al usuario la lista.
6. **Máquina** (informa, no cambies nada sin permiso):
   - RAM y memoria del kernel con `scripts/revisar-recursos`.
